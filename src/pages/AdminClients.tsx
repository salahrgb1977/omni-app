  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-slate-900">Clients CRM</h1>
          <p className="text-slate-500 mt-1 font-medium">Manage client relationships and VIP memberships.</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {loading ? (
                ...
                clients.map((client) => (
                  <tr key={client.id} className="border-b border-slate-100 hover:bg-slate-50 transition-all duration-200">
                    <td className="p-4 font-bold text-slate-900">{client.full_name}</td>
                    <td className="p-4 text-slate-600 font-medium">{client.phone_number || 'N/A'}</td>
                    <td className="p-4 text-slate-600 font-medium">{client.address_text || 'N/A'}</td>
                    <td className="p-4">
                      {client.is_vip_member ? (
                        <div className="flex flex-col">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-yellow-50 text-yellow-600 ring-1 ring-yellow-600/20 w-max">
                    ...
                    <td className="p-4">
                      <button 
                        ...
                        className="text-indigo-600 hover:text-indigo-800 p-2 hover:bg-indigo-50 rounded-lg transition-colors font-medium"
                        title="Edit Client"
                      >
                        <Edit size={16} />
                      </button>
