          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h3 className="text-xs font-bold uppercase tracking-wider">
                Daily Captain Material Deduction
              </h3>
              <span className="text-[10px] text-slate-400">
                Log parts used for billing and inventory sync
              </span>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {successMsg ? (
            <div className="py-8 text-center space-y-2">
              ...
              <p className="text-sm font-bold text-slate-900">Deduction Logged Successfully</p>
              <p className="text-xs text-slate-500">Van inventory and job materials updated.</p>
            </div>
          ) : (
            <>
              {/* Van Selector */}
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`py-2 px-3 rounded-lg border text-xs font-bold flex items-center justify-center space-x-1.5 transition-colors ${
                  ...
                  >
                    <Truck size={14} />
                    <span>Van 1</span>
                    {defaultVehicle === 'van_1' && <span className="text-[10px] opacity-75">(My Van)</span>}
                  </button>
                  ...
                  >
                    <Truck size={14} />
                    <span>Van 2</span>
                    {defaultVehicle === 'van_2' && <span className="text-[10px] opacity-75">(My Van)</span>}
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                {selectedItem && (
                  <p className="text-[11px] text-slate-500 mt-1">
                    Current Van Stock: <strong className="font-mono text-slate-800">{selectedItem.quantity}</strong>
                    {selectedItem.quantity <= 0 && (
                      <span className="text-amber-600 ml-1.5">(Will record negative stock)</span>
                    )}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900"
                >
                  <option value="">-- General Van Consumption (No specific job) --</option>
                  {jobs.map(j => (
              ...
              <div className="pt-2 flex items-center justify-end space-x-2">
                ...
                >
                  {isSubmitting ? 'Logging...' : 'Confirm Material Deduction'}
                </button>
              </div>
            </>
          )}
